"""
screener.py
OpenD市場全体スクリーニング（Stage 1: デイトレード有望候補の絞り込み）。
swing_backtest_studio/screener.py のデイトレード向けフォーク。

「有望性」の根拠は個々の銘柄が構造的にデイトレード戦略（当日〜数日以内の
値幅を取る）に適しているかどうかであり、中核条件は流動性（時価総額・
売買代金）とボラティリティ（値幅）に置く。値幅の算出期間はスイング版
（5/10/20営業日）より短い1/3/5営業日をデフォルトの選択肢とする——
直近の短期的なボラティリティを見るため。

MACDゴールデンクロス（直近で発生中）は「今のトレンドに乗る」ための
補助条件として扱い、必須にはしない（require_gold_cross=Falseが既定）。
MACD Traderの標準足種（K_1M）に合わせ、ゴールデンクロス判定の足種も
K_1Mをデフォルトにする（スイング版はK_DAYがデフォルト）。

ここで絞り込んだ候補は、あくまでStage 2（MACD Backtest Studioの
...
_GOLD_CROSS_PERIOD_MAP = {
    "K_1M": Period.MINUTE_1, "K_5M": Period.MINUTE_5, "K_15M": Period.MINUTE_15,
    "K_30M": Period.MINUTE_30, "K_60M": Period.HOUR_1, "K_DAY": Period.DAY,
}
...
def discover_candidates(
    ...
    gold_cross_period = _GOLD_CROSS_PERIOD_MAP.get(gold_cross_timeframe, Period.MINUTE_1)
